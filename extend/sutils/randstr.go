...
func RandomAlphanumeric(count uint) string {
	// it will crash when calling too many times
	// return RandomAlphaOrNumeric(count, true, true)

	alphanumTable := "ABCDEFGHIJKLMNOPQRSTUVWXYXabcdefghijklmnopqrstuvwxyz01234567890"
	r := rand.New(rand.NewSource(time.Now().UnixNano()))
	alen := len(alphanumTable)
	...
	for i := 0; i < int(count); i++ {
		...
		rstr = rstr + string(alphanumTable[rm])
	}
